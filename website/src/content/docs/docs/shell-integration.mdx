...
`spells init <shell>` writes a snippet to **stdout** — it never edits your dotfiles. Pick your shell:

<Tabs syncKey="shell">
  <TabItem label="zsh">
    Append to `~/.zshrc`, or write to a fragment under `~/.config/spellbook/init.zsh` and `source` it from `~/.zshrc`.

    ```sh frame="terminal"
    spells init zsh >> ~/.zshrc
    ```
  </TabItem>
  <TabItem label="bash">
    Append to `~/.bashrc` (Linux) or `~/.bash_profile` (macOS interactive login).

    ```sh frame="terminal"
    spells init bash >> ~/.bashrc
    ```
  </TabItem>
  <TabItem label="fish">
    Write to `~/.config/fish/conf.d/spellbook.fish` — fish auto-loads conf.d snippets at startup.

    ```sh frame="terminal"
    spells init fish > ~/.config/fish/conf.d/spellbook.fish
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="shell">
  <TabItem label="zsh">
    ```sh frame="terminal"
    bindkey '^I'   # should reference the Spellbook widget
    ```
  </TabItem>
  <TabItem label="bash">
    ```sh frame="terminal"
    bind -p | grep TAB   # should show the Spellbook handler
    ```
  </TabItem>
  <TabItem label="fish">
    ```sh frame="terminal"
    bind \t   # should be in the Spellbook conf.d snippet
    ```
  </TabItem>
</Tabs>
